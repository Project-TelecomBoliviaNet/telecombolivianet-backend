...
public record NotifPlantillaDto(
    ...
    DateTime           CreadoAt,
    // US-META: configuración para Meta template messages
    string?            MetaTemplateName,
    string?            MetaLanguageCode,
    string?            MetaParamOrder     // JSON array: ["nombre","deuda","empresa",...]
);

public record UpdateNotifPlantillaDto(
    string             Texto,
    PlantillaCategoria Categoria        = PlantillaCategoria.General,
    HsmStatus          HsmStatus        = HsmStatus.Pendiente,
    // US-META
    string?            MetaTemplateName = null,
    string?            MetaLanguageCode = "es",
    string?            MetaParamOrder   = null
);
...
// ── US-18 · Delivery status de WhatsApp (enviado por el chatbot) ────────────

public record WhatsappDeliveryStatusDto(
    string WaMessageId,
