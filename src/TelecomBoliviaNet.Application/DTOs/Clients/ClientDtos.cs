...
public record UpdateClientDto(
    ...
    bool    HasTvCable,
    string? OnuSerialNumber,
    // FIX-06: token de concurrencia. 0 = omitir check (backward compat).
    // El cliente debe enviar el valor recibido en ClientDetailDto.RowVersion.
    uint    RowVersion = 0
);
...
public record ClientDetailDto(
    ...
    // FIX-06: token de concurrencia — enviar en UpdateClientDto.RowVersion para control optimista
    uint     RowVersion       // PostgreSQL xmin
);
...
public record RegisterPaymentDto(
    ...
    string?    BankReference      = null    // Nro. de operación / referencia bancaria
);
