...
public record DashboardKpisDto(
    ...
    int     MensajesWspHoy,
    // F1: ARPU — ingreso promedio por cliente activo este mes
    decimal Arpu,
    // F2: bajas del mes para mostrar crecimiento neto
    int     ClientesCanceladosMes
);
...
public record AgingBucketDto(
    string  Rango,          // "0-30" | "31-60" | "61-90" | "+90"
    int     FacturasCount,
    decimal MontoTotal,
    int     ClientesCount
);

public record AgingCarteraDto(
...
public record ClienteProximoCortDto(
    Guid    ClienteId,
    string  ClienteNombre,
    string  Zona,
    decimal MontoDeuda,
    int     DiasParaCorte,    // negativo si ya venció
    string  EstadoFactura
);

public record ProximosCortDto(
    int                         Total,
    List<ClienteProximoCortDto> Clientes
);
...
public record WorkloadTecnicoDto(
    string TecnicoNombre,
    int    TicketsActivos,
    int    TicketsCriticos,
    int    TicketsEnVisita,
    double PromedioHorasAbierto
);

public record WorkloadTecnicosDto(
    int                       TotalSinAsignar,
    List<WorkloadTecnicoDto>  Tecnicos
);
