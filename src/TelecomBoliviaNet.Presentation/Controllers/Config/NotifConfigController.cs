...
public class NotifConfigController : BaseController
{
    ...
    public NotifConfigController(NotifConfigService svc, NotifEnvioService envioSvc)
    {
        _svc      = svc;
        _envioSvc = envioSvc;
    }
    ...
    {
        // Delegado a NotifEnvioService (versión corregida: batch, contexto completo, anti-spam eficiente)
        var result = await _envioSvc.EnvioMasivoAsync(dto, CurrentUserId, CurrentUserName, ClientIp);
        return result.IsSuccess ? OkResult(result.Value) : BadRequestResult(result.ErrorMessage);
    }
}
