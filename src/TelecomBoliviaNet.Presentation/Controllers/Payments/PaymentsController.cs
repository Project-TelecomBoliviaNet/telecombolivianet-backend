...
public class PaymentsController : BaseController
{
    ...
    private readonly IWhatsAppReceiptService   _receiptSvc;
    private readonly PaymentCreditService      _creditSvc;
    private readonly ICashCloseService         _cashCloseSvc;

    public PaymentsController(
        IPaymentService svc,
        IWhatsAppReceiptService receiptSvc,
        PaymentCreditService creditSvc,
        ICashCloseService cashCloseSvc)
    {
        _svc          = svc;
        _receiptSvc   = receiptSvc;
        _creditSvc    = creditSvc;
        _cashCloseSvc = cashCloseSvc;
    }
    ...
    /// <summary>Asigna un cliente a un comprobante huérfano (sin cliente identificado).</summary>
    [HttpPost("whatsapp-queue/{id:guid}/assign-client")]
    [Authorize(Policy = "AdminOnly")]
    public async Task<IActionResult> AssignClient(Guid id, [FromBody] AssignReceiptClientDto dto)
    {
        var result = await _receiptSvc.AssignClientAsync(
            id, dto.ClientId, CurrentUserId, CurrentUserName, ClientIp);

        if (!result.IsSuccess) return BadRequestResult(result.ErrorMessage);
        return OkMessage("Cliente asignado al comprobante correctamente.");
    }
    ...
    {
        var dto = await _cashCloseSvc.GetOrCreateActiveTurnoAsync(CurrentUserId, CurrentUserName);
        return OkResult(dto);
    }
    ...
    {
        var result = await _cashCloseSvc.CerrarTurnoAsync(CurrentUserId, CurrentUserName, ClientIp);
        return result.IsSuccess ? OkResult(result.Value) : BadRequestResult(result.ErrorMessage);
    }
    ...
    {
        var list = await _cashCloseSvc.GetCashClosesAsync(operatorId, desde, hasta);
    ...
    public async Task<IActionResult> GetReportByOperator(
        ...
        [FromQuery] Guid? operatorId)
    {
        var result = await _svc.GetCollectionByOperatorAsync(from, to, operatorId);
        if (!result.IsSuccess) return BadRequestResult(result.ErrorMessage);
        return OkResult(result.Value);
    }
    ...
    public async Task<IActionResult> ExportReportCsv(
        ...
        [FromQuery] Guid? operatorId)
    {
        var reportResult = await _svc.GetCollectionByOperatorAsync(from, to, operatorId);
