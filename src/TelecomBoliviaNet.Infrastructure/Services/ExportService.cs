...
using TelecomBoliviaNet.Application.Utils;
...
public class ExportService : IExportService
{
        ...
        InvoiceMonthStatsDto stats)
    {
        ...
        // Línea separadora
        ws.Range(mRow + 2, 1, mRow + 2, 13).Style.Border.BottomBorder = XLBorderStyleValues.Thin;
        ws.Range(mRow + 2, 1, mRow + 2, 13).Style.Border.BottomBorderColor = XLColor.FromHtml("#E5E7EB");

        // ── Cabecera de tabla ──────────────────────────────────────────────────
        // Columnas: N°Factura | TBN | Cliente | Zona | Plan | Estado | Monto | Monto Cobrado | Días Mora | Notas | Emisión | Vencimiento | Fecha Pago
        int row = mRow + 4;
        string[] headers = ["N° Factura", "Código TBN", "Cliente", "Zona", "Plan", "Estado",
                             "Monto (Bs.)", "Monto Cobrado (Bs.)", "Días Mora", "Notas", "Emisión", "Vencimiento", "Fecha Pago"];
        ...
        foreach (var inv in list)
        {
            ...
            ws.Cell(row, 2).Value = inv.TbnCode;
            ws.Cell(row, 2).Style.Font.Bold = true;
            ws.Cell(row, 2).Style.Font.FontColor = XLColor.FromHtml("#1D4ED8");

            ws.Cell(row, 3).Value = inv.ClientName;
            ws.Cell(row, 4).Value = inv.Zone;
            ws.Cell(row, 4).Style.Font.FontColor = XLColor.FromHtml("#6B7280");
            ws.Cell(row, 5).Value = inv.PlanName;

            ws.Cell(row, 6).Value = inv.Status;
            ws.Cell(row, 6).Style.Fill.BackgroundColor = statusBg;
            ws.Cell(row, 6).Style.Font.Bold = true;

            ws.Cell(row, 7).Value = inv.Amount;
            ws.Cell(row, 7).Style.NumberFormat.Format = "#,##0.00";
            ws.Cell(row, 7).Style.Font.Bold = true;

            // Monto Cobrado: total si Pagada, parcial si ParcialmentePagada, 0 si sin cobrar
            var montoCobrado = inv.Status switch
            {
                "Pagada"             => inv.Amount,
                "ParcialmentePagada" => inv.AmountPaid,
                _                    => 0m
            };
            if (montoCobrado > 0)
                ws.Cell(row, 8).Value = montoCobrado;
            else
                ws.Cell(row, 8).Value = "—";
            ws.Cell(row, 8).Style.NumberFormat.Format = "#,##0.00";
            ws.Cell(row, 8).Style.Font.FontColor = XLColor.FromHtml("#16A34A");

            if (inv.DaysOverdue > 0)
            {
                ws.Cell(row, 9).Value = inv.DaysOverdue;
            ...
            ws.Cell(row, 10).Value = inv.Notes ?? "";
            ws.Cell(row, 10).Style.Font.FontColor = XLColor.FromHtml("#6B7280");
            ws.Cell(row, 11).Value = inv.IssuedAt.ToString("dd/MM/yyyy");
            ws.Cell(row, 12).Value = inv.DueDate.ToString("dd/MM/yyyy");
            ws.Cell(row, 13).Value = inv.PaidAt.HasValue ? inv.PaidAt.Value.ToString("dd/MM/yyyy") : "";
            ws.Cell(row, 13).Style.Font.FontColor = XLColor.FromHtml("#16A34A");

            // Fondo alternado en columnas que no tienen color de estado
            foreach (int col in new[] { 1, 2, 3, 4, 5, 7, 8, 9, 10, 11, 12, 13 })
                ws.Cell(row, col).Style.Fill.BackgroundColor = rowBg;

            // Bordes inferiores
            ws.Range(row, 1, row, 13).Style.Border.BottomBorder = XLBorderStyleValues.Thin;
            ws.Range(row, 1, row, 13).Style.Border.BottomBorderColor = XLColor.FromHtml("#E5E7EB");
        }

        // ── Totales ───────────────────────────────────────────────────────────
        row += 2;
        ws.Cell(row, 6).Value = "TOTAL FACTURADO:";
        ws.Cell(row, 6).Style.Font.Bold = true;
        ws.Cell(row, 6).Style.Font.FontColor = XLColor.FromHtml("#1D4ED8");
        ws.Cell(row, 7).Value = stats.TotalBilled;
        ws.Cell(row, 7).Style.NumberFormat.Format = "#,##0.00";
        ws.Cell(row, 7).Style.Font.Bold = true;
        ws.Cell(row, 7).Style.Font.FontColor = XLColor.FromHtml("#1D4ED8");

        row++;
        ws.Cell(row, 6).Value = "TOTAL COBRADO:";
        ws.Cell(row, 6).Style.Font.Bold = true;
        ws.Cell(row, 6).Style.Font.FontColor = XLColor.FromHtml("#16A34A");
        ws.Cell(row, 7).Value = stats.TotalCollected;
        ws.Cell(row, 7).Style.NumberFormat.Format = "#,##0.00";
        ws.Cell(row, 7).Style.Font.Bold = true;
        ...
        InvoiceMonthStatsDto stats)
    {
        ...
        return Document.Create(container =>
        {
            container.Page(page =>
            {
                ...
                // ── Tabla de facturas ────────────────────────────────────────
                // Columnas: Código | Cliente | Zona | Plan | Estado | Monto | Pagado | Emisión | Vencimiento | Fecha Pago
                page.Content().PaddingTop(8).Table(table =>
                {
                    table.ColumnsDefinition(cols =>
                    {
                        cols.ConstantColumn(52);   // Código TBN
                        cols.RelativeColumn(2.5f); // Cliente
                        cols.RelativeColumn(1.2f); // Zona
                        cols.RelativeColumn(1.8f); // Plan
                        cols.ConstantColumn(65);   // Estado
                        cols.ConstantColumn(68);   // Monto
                        cols.ConstantColumn(68);   // Pagado
                        cols.ConstantColumn(58);   // Emisión
                        cols.ConstantColumn(58);   // Vencimiento
                        cols.ConstantColumn(60);   // Fecha Pago
                    });
                    ...
                    table.Header(h =>
                    {
                        foreach (var label in new[] { "Código", "Cliente", "Zona", "Plan", "Estado", "Monto (Bs.)", "Monto Cobrado (Bs.)", "Emisión", "Vencimiento", "Fecha Pago" })
                    ...
                    foreach (var inv in list)
                    {
                        ...
                        table.Cell().Element(DataCell)
                            .Text(inv.Zone).FontSize(8).FontColor("#4B5563");
                        table.Cell().Element(DataCell)
                        ...
                        var pdfMontoCobrado = inv.Status switch
                        {
                            "Pagada"             => inv.Amount,
                            "ParcialmentePagada" => inv.AmountPaid,
                            _                    => 0m
                        ...
                        table.Cell().Element(DataCell)
                            .Text(pdfMontoCobrado > 0 ? $"Bs. {pdfMontoCobrado:N2}" : "—").FontSize(8).FontColor("#16A34A");
                        table.Cell().Element(DataCell)
                        ...
                        table.Cell().Element(DataCell)
                            .Text(inv.PaidAt.HasValue ? inv.PaidAt.Value.ToString("dd/MM/yyyy") : "—")
                            .FontSize(8).FontColor(inv.PaidAt.HasValue ? "#16A34A" : "#9CA3AF");
                    }
                });
    ...
    private static string MonthName(int m) => InvoiceHelper.MonthName(m);
}
