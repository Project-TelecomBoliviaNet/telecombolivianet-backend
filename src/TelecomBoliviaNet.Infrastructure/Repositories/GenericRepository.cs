...
public class GenericRepository<T> : IGenericRepository<T> where T : Entity
{
    ...
    /// <summary>
    /// Marca la entidad como Modified en el ChangeTracker sin llamar SaveChangesAsync.
    /// Diseñado para usarse dentro de transacciones IUnitOfWork donde todas las
    /// operaciones se acumulan y se persisten con un único SaveChangesAsync antes del CommitAsync.
    /// Esto garantiza que el ChangeTracker no genera saves parciales intermedios que
    /// podrían dejar la BD en estado inconsistente si un paso posterior falla.
    /// </summary>
    public Task MarkModifiedAsync(T entity)
    {
    ...
    public async Task UpdateAsync(T entity)
    {
        ...
        if (tracked is not null)
        {
            ...
            // No cambiar estado si ya es Added: la entidad aún no fue INSERTada;
            // forzar Modified causaría DbUpdateConcurrencyException (0 rows affected).
            tracked.CurrentValues.SetValues(entity);
            if (tracked.State != EntityState.Added)
                tracked.State = EntityState.Modified;
        }
