...
public class BillingBackgroundJob : BackgroundService
{
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        async Task<string> ReadLastRun(string key, IServiceScope scope)
        {
            var appDb = scope.ServiceProvider.GetRequiredService<TelecomBoliviaNet.Infrastructure.Data.AppDbContext>();
            return await appDb.SystemConfigs.Where(c => c.Key == key).Select(c => c.Value).FirstOrDefaultAsync(stoppingToken) ?? "";
        }
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                if (day == 1 && hour == 0 && min >= 1 && min <= 10)
                {
                    ...
                    if (await ReadLastRun(SystemConfigKeys.BillingLastGeneratedMonth, scope) != monthKey)
                    {
                        ...
                        await WriteLastRun(SystemConfigKeys.BillingLastGeneratedMonth, monthKey, scope);
                    }
                }
                ...
                if (day == 7 && hour == 9 && min >= 1 && min <= 10)
                {
                    ...
                    if (await ReadLastRun(SystemConfigKeys.BillingLastReminderMonth, scopeR) != monthKey)
                    {
                        ...
                        await WriteLastRun(SystemConfigKeys.BillingLastReminderMonth, monthKey, scopeR);
                    }
                }
                ...
                {
                    using var scope = _services.CreateScope();
                    if (await ReadLastRun(SystemConfigKeys.BillingLastVoidedMonth, scope) != monthKey)
                    {
                        _logger.LogInformation("Ejecutando job de VENCIMIENTO {Month}/{Year}", month, year);
                        var billing = scope.ServiceProvider.GetRequiredService<BillingService>();
                        await billing.MarkOverdueInvoicesAsync();
                        await WriteLastRun(SystemConfigKeys.BillingLastVoidedMonth, monthKey, scope);
                    }
                }
            }
            catch (Exception ex)
